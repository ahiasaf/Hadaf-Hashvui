/* ============================================================
   ...
   **היא אינה שער ואינה אישור שצריך לתת.** ההגרלה נרשמה כבר
   ...
   הוא אינו יודע את המזהה של אבא — אבא נרשם במכשיר שלו, עם
   מזהה משלו. מה שמחבר ביניהם הוא **הטלפון**: הבן מסר אותו
   בהרשמה (חובה, מאז שהוא מה שמחבר), ואבא נרשם איתו. לכן:
   טלפון → שורה ב"לומדים" → מזהה → מנוי ב"התראות".

   מי שההורה שלו לא התקין — ההודעה פשוט אינה יוצאת. השורה
  ...
  .then(function (r) {
    ...
    return due.reduce(function (chain, o) {
      return chain.then(function (res) {
        ...
        var pk = phoneKey(o['טלפון ההורה']);
        var pid = pk && byPhone[pk];
        if (!pid) {
          console.log('  · ' + name + ' — ההורה עדיין לא נרשם, ממתין');
          res.push(0); return res;
        }
        var raw = last[pid];
        if (!raw) {
          console.log('  · ' + name + ' — ההורה נרשם ולא התקין, ממתין');
          res.push(0); return res;
        }
        var sub;
        ...
        var body = fill(T.pushB, { name: name, daf: daf });
        if (DRY) {
