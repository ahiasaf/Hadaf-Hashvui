...
var ASK = (function () {
  ...
  /* המקום שבו הבקשה יושבת כרגע. היא נודדת בין מסכים — מסך
     ההרשמה, אזור הניהול — ובלי לנקות את הקודם נשארים שני
     עותקים של אותם מזהים במסמך. `getElementById` מחזיר את
     הראשון, ולכן הלחיצות היו מגיעות לעותק שאיש אינו רואה. */
  var HOST = null;

  function mount(where) {
    var host = $(where);
    if (!host) return;
    if (HOST && HOST !== host && HOST.parentNode) HOST.innerHTML = '';
    HOST = host;
