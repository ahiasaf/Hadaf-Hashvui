...
var GUIDE_UI = (function () {
  ...
  /* ============================================================
     ...
     נכון, ולכן חמישה מהשלבים הם צילום מסך ולא ציור. ציור,
     טוב ככל שיהיה, הוא תמיד פרשנות; צילום הוא בדיוק מה שהם
     יראו — אותו גופן, אותו סידור, אותה מילה.

     `x`/`y` באחוזים הם מרכז הטבעת על התמונה. הם באחוזים ולא
     בפיקסלים כדי שהסימון יישאר במקומו בכל רוחב מסך.
     ...
     **מה שאינו כאן ולמה.** הצילום של גיליון השיתוף המלא נושא
     ...
     ============================================================ */
  var PICS = {
    n1:  { src:'guidepics/n1.webp',  x:15,  y:41, r:7.4 },
    n2:  { src:'guidepics/n2.webp',  x:79,  y:10, r:5.6 },
  ...
  function pic(p) {
    return '<div class="gu-pic"><span><img src="' + p.src + '" alt="" loading="lazy">' +
      /* **`left` ולא `inset-inline-start`.** האחוזים נמדדים על
         התמונה, שהיא תמונה ולא טקסט — ובעמוד מימין לשמאל
         `inset-inline-start` הוא הצד הימני, כלומר הסימון נחת
         בדיוק בצד ההפוך מהכפתור. */
      '<i class="gu-dot" style="left:' + p.x + '%;top:' + p.y +
      '%;width:' + (p.r * 2) + '%;padding-block-end:' + (p.r * 2) + '%"></i></span></div>';
  }
  ...
  function css() {
    ...
    st.textContent = [
      ...
      /* הסימון על הצילום. עיגול ולא מסגרת: על תמונה צפופה
         מסגרת מרובעת נבלעת בין השורות. */
      /* ============================================================
         **אנימציה משלו, ולא `guPulse`.**
         ============================================================
         `guPulse` מנפיש `transform: scale()`, והוא דורס את
         ה-`translate(-50%,-50%)` שממרכז את הסימון על הנקודה.
         התוצאה: הטבעת נחתה חצי־רוחב מתחת ומימין למה שהיא
         אמורה לסמן — כלומר על הכפתור השכן.
         ============================================================ */
      '@keyframes guDot{0%,100%{transform:translate(-50%,-50%) scale(1);opacity:1}',
      '  50%{transform:translate(-50%,-50%) scale(1.07);opacity:.62}}',
      '.gu-dot{position:absolute;transform:translate(-50%,-50%);',
      '  border-radius:50%;border:2.5px solid var(--gold);',
      '  box-shadow:0 0 0 4px rgba(229,184,84,.28);',
      '  animation:guDot 1.9s ease-in-out infinite;pointer-events:none}',
      '@media (prefers-reduced-motion:reduce){.gu-dot{animation:none}}',
